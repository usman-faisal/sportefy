...
export class SlotService {
  ...
  async validateTimeSlot(
    ...
    if (userId) {
      const userOverlappingBookings = await this.slotRepository.getManySlots(
        and(
    ...
    await this.checkTimeConflicts(startTime, endTime, venue, options);
  }
  ...
  private async checkOperatingHours(
    ...
    if (relevantHourIntervals.length === 0) {
      throw new BadRequestException(
        `The venue is closed on ${startDayOfWeek}s.`,
      );
    }
  ...
  private async checkTimeConflicts(
    ...
    const slotsToCheck = overlappingSlots.filter((slot) => {
    ...
    const hasConflict = slotsToCheck.some((slot) => {
      ...
      const hasTimeOverlap = startTime < itemEnd && endTime > itemStart;

      if (!hasTimeOverlap) return false;

      if (slot.eventType === 'booking' && slot.booking) {
        return slot.booking.status === 'confirmed';
      }
    ...
    if (hasConflict) {
      throw new ConflictException(
        'The requested time slot conflicts with a confirmed booking or maintenance schedule.',
      );
    }
  }
